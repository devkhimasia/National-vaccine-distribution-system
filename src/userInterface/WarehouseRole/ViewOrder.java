...
public class ViewOrder extends javax.swing.JPanel {
    ...
    public ViewOrder(JPanel userProcessContainer, ClinicOrderRequest cor, WarehouseOrganization o, DistributorOrganization distributorOrganization) {
        initComponents();
...
//throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    ...
    public void populateOrderTbl() {
        ...
        dtm.setRowCount(0);
        if (cor.getOrder().getStatus() != "Ready for Shipment") {
    ...
    private void initComponents() {
        ...
        gridBagConstraints.anchor = java.awt.GridBagConstraints.NORTHWEST;
        gridBagConstraints.insets = new java.awt.Insets(29, 76, 13, 0);
...
public void populateSecondTable(int s) {
        ...
        dtm.setRowCount(0);
        Vaccine v = (Vaccine) orderTbl.getValueAt(s, 0);
...
//                            OrderItem oii = it.next();
    ...
    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
        int itemp = cor.getOrder().getOrder().size();
        ArrayList<tempVaccine> tempVaccineList = new ArrayList<>();
        for (OrderItem oi : tempOrder.getOrder()) {
            Boolean f2 = true;
            for (tempVaccine tv : tempVaccineList) {
                if (tv.getVaccine().equals(oi.getProduct())) {
                    // System.out.println("Dev1");
                    int a = tv.getQuantity();
                    tv.setQuantity(a + oi.getQuantity());
                    f2 = false;
                    break;
                } else {
                    // System.out.println("Dev2");
                    tempVaccine tv1 = new tempVaccine();
                    tv1.setVaccine(oi.getProduct());
                    tv1.setQuantity(oi.getQuantity());
                    tempVaccineList.add(tv1);
                    f2 = false;
                    break;
                }
            }
        ...
        if (itemp != tempVaccineList.size()) {
            JOptionPane.showMessageDialog(null, "Items doesnt not match");
            for (OrderItem oi : tempOrder.getOrder()) {
                for (Inventory i : o.getInventoryCatalog().getInventoryList()) {
                    if (oi.getProduct().equals(i.getVaccine())) {
                        for (InventoryItem ii : i.getInventoryItemList()) {
            ...
            for (OrderItem oi : cor.getOrder().getOrder()) {
                for (tempVaccine tv : tempVaccineList) {
                    if (oi.getProduct().equals(tv.getVaccine())) {
                        if (oi.getQuantity() != tv.getQuantity()) {
                            JOptionPane.showMessageDialog(null, "Quantities does not match");
                            for (OrderItem oi1 : tempOrder.getOrder()) {
                                for (Inventory i : o.getInventoryCatalog().getInventoryList()) {
                                    if (oi1.getProduct().equals(i.getVaccine())) {
                                        for (InventoryItem ii : i.getInventoryItemList()) {
                                            ii.setTempQuantity(0);
                                        }
                                    }
            ...
            for (OrderItem oi : cor.getOrder().getOrder()) {
                oi.setExpiryDate(null);
            }
            cor.setShippedOrder(tempOrder);
            cor.getOrder().setStatus("Ready for Shipment");
            cor.setStatus("Ready for Shipment");
            WarehouseShipmentRequest wsr = new WarehouseShipmentRequest();
            wsr.setCor(cor);
            wsr.setSender(o);
            wsr.setStatus("Forwarded to Shipment");
            for (Organization org : distributorOrganization.getOrganizationDirectory().getOrganizationList()) {
                if (org instanceof ShipperOrganization) {
                    ShipperOrganization so = (ShipperOrganization) org;
                    so.getWorkQueue().getWorkRequestList().add(wsr);
                }
            }
            if(cor.getOrder().getTotalAmount()>0){
            PaymentRequest pr = new PaymentRequest();
            pr.setSender(distributorOrganization);
            pr.setStatus("Invoice forwarded to site");
            pr.setOrder(cor.getOrder());
            
                pr.setReceiver(cor.getSender());
                cor.getSender().getWorkQueue().getWorkRequestList().add(pr);
            
            
            distributorOrganization.getWorkQueue().getWorkRequestList().add(pr);
            }
populateOrderTbl();
        }
